...
| 2026-09-12 | 7 | P1.8 | Minh asked for note labels in Vietnamese solfège instead of letter names, mid-checkpoint of P1.5. Answered open question Q3, recorded as DR-8, added task P1.8 to `docs/PLAN.md` (branched off `p1.5-editor` rather than `main`, since PR #7/P1.5 was still open — kept as its own branch/PR, `p1.8-solfege-names`, based on `main`, to avoid duplicating P1.5's unmerged content). Added `midiToSolfege` (fixed-do: Đô=C, Rê=D, ...) to `src/notes/mapping.ts`, tests written first in `test/notes/mapping.test.ts` (same edge cases as `midiToName`, plus all 12 pitch classes), red → green. Wired into `main.ts`'s hover label to show `"Đô4 (C4)"` (solfège default, letter name alongside). Also logged a parked idea (constrain the editor to a musical scale) — recommended against for v1, not built. `npm test` green (38 tests), `tsc --noEmit` and `npm run build` clean. Ran `npm run dev`; Minh checkpoint passed ("Có, chính xác rồi."). | On estimate (1 session, within the 0.5 estimate — rounds up since it's a discrete session) | Once PR #7 (P1.5) and this PR (P1.8) are both merged: P1.6 — seed the 8-note C-major-scale melody, "Clear all" and "Play" buttons; Phase 1 final checkpoint |
