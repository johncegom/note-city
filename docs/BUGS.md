...
**Status:** fix implemented in P1.10 (2026-09-12): widened `#skyline` from 500px/5s to 700px/7s, and clamped `addNoteAt`'s `start` and the duration-drag's cap so a note can never extend past the canvas's own width. Verified via dispatched DOM events (a note placed near the new right edge renders fully, and dragging further right stays clamped instead of extending invisibly past the edge); pending Minh's own checkpoint confirmation in the running app before marking P1.10 done.
