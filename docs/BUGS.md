...
**Status:** decided (2026-09-17): option (b), scoped to `playAll` (skyline's "Play" button, also used by "Play synth") and `playOriginal` ("Play original audio") — not the short single-note previews (Play C4/C5, click/drag preview). Fixing in P2.5's branch since it's the same session that surfaced it.

## BUG-5: "Play synth" / skyline "Play" produce no audible sound on a long real song

**Symptom:** Minh reported that after loading a real ~270s song ("LAVIEM.mp3", 2336 notes found), "Play original audio" worked fine, but "Play synth" and the skyline's "Play" (both call `playAll`) produced no sound at all.
**Root cause:** confirmed via reproduction (synthetic tests in the browser, not guesswork — see `docs/PLAN.md` DR-12). `playAll` (`src/main.ts`) created one `OscillatorNode`/`GainNode` pair per note *up front*, synchronously, before any of them could produce sound. For 2336 notes this measurably delayed when the audio graph actually started outputting sound: a synthetic test with 500 notes over 60s produced audible signal within ~100ms, but 2336 notes over 270s stayed silent for ~900ms+ before the first sound. `playOriginal` doesn't have this issue — it plays one pre-rendered `AudioBufferSourceNode`, not one node per note, so its cost doesn't scale with note count. Not a crash, not a compressor/overlap issue (an `OfflineAudioContext` render with 200 heavily-overlapping notes through the same compressor setup produced clear signal, ruling that out) — purely the up-front node-creation cost.
**Reachability:** real path — any real song with enough notes (roughly proportional to its length) hits this; Minh's actual test file (4.5x the app's previous ≤60s design target) made it severe enough to look like total silence.
**Options:** (a) cap input length (the app's original ≤60s target) so note counts stay small enough that up-front scheduling is fast enough to be imperceptible; (b) fix `playAll` with a lookahead scheduler — only create oscillators for notes due in the next couple of seconds, topping up on every animation-frame tick, so live node count stays small regardless of song length (standard Web Audio pattern, not novel).
**Status:** fixed in P2.5's branch (2026-09-17), option (b) — Minh decided the product should support up to 5-minute inputs (see DR-12), so capping duration wasn't the right fix. `playAll` now uses a `SCHEDULE_AHEAD_SEC = 2` lookahead: `scheduleDueEvents()` walks the already-sorted `ScheduledEvent[]` and only creates oscillators for events due within the next 2 seconds, called once up front and again on every `tick()`. Verified via the same synthetic reproduction: with the lookahead scheduler, 2336 notes over 270s no longer show the startup delay (only ~17 notes are ever scheduled at once instead of 2336).
